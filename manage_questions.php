    ...
    <div class="max-w-5xl mx-auto py-6 space-y-4">
        ...
        <div class="max-w-3xl">
            <section class="rounded-2xl border border-slate-200 bg-white p-5 space-y-3">
                ...
                <p class="text-xs text-slate-600"><strong>SQL practical:</strong> JSON uses <code class="rounded bg-slate-100 px-1">setup_sql</code> (schema/seed), <code class="rounded bg-slate-100 px-1">reference_sql</code> (usually a SELECT whose rows are compared — lenient overlap). Students may run SELECT, INSERT, UPDATE, etc., in one statement. For INSERT/DML homework, add <code class="rounded bg-slate-100 px-1">golden_sql</code> with the instructor solution statement so grading can compare <code class="rounded bg-slate-100 px-1">reference_sql</code> after the student&apos;s script vs after <code class="rounded bg-slate-100 px-1">golden_sql</code>. Optional <code class="rounded bg-slate-100 px-1">hints</code>.</p>
            ...
            <div class="space-y-3 max-h-[50vh] overflow-auto">
                <?php foreach ($pendingQuestions as $q): ?>
                    <form method="post" class="border rounded-lg p-3 space-y-2 grid gap-2 md:grid-cols-2">
                        ...
                        <label class="md:col-span-2 text-xs text-slate-500">SQL practical JSON (<code class="rounded bg-slate-100 px-1">setup_sql</code>, <code class="rounded bg-slate-100 px-1">reference_sql</code>, optional <code class="rounded bg-slate-100 px-1">golden_sql</code> for DML)</label>
                        <textarea name="sql_practice" rows="8" class="md:col-span-2 w-full border rounded px-2 py-1.5 font-mono text-[11px] leading-relaxed" placeholder='{"setup_sql":"CREATE TABLE Products(...);","reference_sql":"SELECT * FROM Products ORDER BY ProductID","golden_sql":"INSERT INTO Products ..."}'><?php echo htmlspecialchars((string) ($q['sql_practice'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></textarea>
                        <div class="flex flex-wrap gap-2">
