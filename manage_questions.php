...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_edit') {
        ...
        if ($id < 1 || $selectedQuizId < 1) {
            ...
            $error = 'Edit failed. Correct answer is required.';
        } else {
...
/** Stored question_type for admin edits (MCQ / fill / theory only). */
function trytest_manage_questions_edit_type_value(array $q): string
{
    $t = strtolower(trim((string) ($q['question_type'] ?? '')));
    if (in_array($t, ['fill', 'theory'], true)) {
        return $t;
    }
            ...
            <div class="space-y-3 max-h-[50vh] overflow-auto">
                <?php foreach ($pendingQuestions as $q): ?>
                    <form method="post" class="border rounded-lg p-3 space-y-2 grid gap-2 md:grid-cols-2">
                        ...
                        <input type="hidden" name="question_type" value="<?php echo htmlspecialchars(trytest_manage_questions_edit_type_value($q), ENT_QUOTES, 'UTF-8'); ?>">
                        <div class="grid gap-2 md:grid-cols-2">
                            ...
                            <input class="border rounded px-2 py-1.5" name="correct_answer" value="<?php echo htmlspecialchars((string) $q['correct_answer'], ENT_QUOTES, 'UTF-8'); ?>" required placeholder="Model answer">
